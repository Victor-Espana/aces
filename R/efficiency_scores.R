...
#'
#' @description This function calculates the efficiency scores for each Decision-Making-Unit through an Adaptive Constrained Enveloping Splines model and a specific technology determined by the user.
#'
#' @param tech_data A \code{data.frame} or a \code{matrix} containing the observed DMUs to determine the technology.
#' @param eval_data A \code{data.frame} or a \code{matrix} containing the DMUs to be evaluated.
#' @param x Column indexes of input variables in \code{tech_data} and \code{eval_data}.
#' @param y Column indexes of output and netput variables in \code{tech_data} and \code{eval_data}.
#' @param addi_x Column indexes of additional covariables in \code{data}.
#' @param object An \code{aces} object.
#' @param method Model prediction used to compute predictions of inputs and obtain a new vector of outputs for \code{eval_data} .
#' \itemize{
...
#' }
#' @param proximity A value indicating the proximity of the DMUs to the DEA-VRS frontier.
#' @param measure Mathematical programming model to calculate scores:
#' \itemize{
...
#' }
#' @param convexity A \code{logical} value indicating if a convex technology is assumed. Default is \code{TRUE}.
#' @param returns Type of returns to scale:
#' \itemize{
#' \item{\code{"variable"}} Variable returns to scale (default).
#' \item{\code{"constant"}} Constant returns to scale.
#' }
#' @param direction Only applied if \code{measure = "ddf"}. Direction of the vector to project on the frontier:
#' \itemize{
...
#' \item{\code{"BAM"}} Bounded Adjusted Measure.
#' }
#' @param digits Number of decimal places to which efficiency scores are rounded.
#'
...
#' @export
#'
#' @return A \code{data.frame} with the efficiency scores computed through an Adaptive Constrained Enveloping Splines model.

aces_scores <- function (
    tech_data, eval_data, x, y, addi_x, object, method,
    proximity = ifelse(length(y) == 1, Inf, 0.05),
    measure = "rad_out", convexity = TRUE, returns = "variable",
    direction = NULL, weights = NULL,
    digits = 3
    ) {
