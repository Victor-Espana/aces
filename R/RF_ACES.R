...
rf_aces_algorithm <- function (
  ...
  # ================================================================== #
  #  STOCHASTIC RANDOM FOREST ADAPTIVE CONSTRAINED ENVELOPING SPLINES  #
  # ================================================================== #

  if (model_type == "sto") {

    for (model in c("rf_aces", "rf_aces_cubic", "rf_aces_quintic")) {
      ...
      if (model == "rf_aces") {
        y_hat <- rf_aces[["Bmatx"]] %*% rf_aces[["coefs"]]

      } else if (model == "rf_aces_cubic") {
        y_hat <- rf_aces_cubic[["Bmatx"]] %*% rf_aces_cubic[["coefs"]]

      } else {
        y_hat <- rf_aces_quintic[["Bmatx"]] %*% rf_aces_quintic[["coefs"]]
      ...
      if (model == "rf_aces") {
        rf_aces[["sto"]] <- stochastic_analysis

      } else if (model == "rf_aces_cubic") {
        rf_aces_cubic[["sto"]] <- stochastic_analysis

      } else {
        rf_aces_quintic[["sto"]] <- stochastic_analysis
